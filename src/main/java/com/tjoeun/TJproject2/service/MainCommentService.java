...
import com.tjoeun.TJproject2.dao.MainCommentDAO;
import com.tjoeun.TJproject2.mybatis.MySession;
import com.tjoeun.TJproject2.vo.MainCommentList;
...
public class MainCommentService {
	...
	// MainCommentList.jsp에서 호출되는 mapper를 얻어온 후 MainCommentDAO 클래스의 테이블에 저장된 
	...
	public void delete(int idx) {
		System.out.println("MainCommentService 클래스의 delete() 메소드 실행");
		SqlSession mapper = MySession.getSession();
		
		MainCommentDAO.getInstance().delete(mapper, idx);
	...
	public void deleteMessage(int idx) {
		System.out.println("MainCommentService 클래스의 deleteMessage() 메소드 실행");
		SqlSession mapper = MySession.getSession();
		
		MainCommentDAO.getInstance().deleteMessage(mapper, idx);
